...
INTEGER::i,j,k,x,y
Integer,ALLOCATABLE ::rede(:,:),vizinhos(:,:,:),quantidade_loops
REAL*8:: beta, temperatura,aleatorio, energia, obterEnergia,dE
REAL*8,ALLOCATABLE :: serie_temp_energia(:)
ALLOCATE(rede(1:n,1:n),vizinhos(1:n,1:n,1:4))
energia=0.0
rede(:,:)=-1
quantidade_loops=1000000
ALLOCATE(serie_temp_energia(1:quantidade_loops))
call sgrnd(getseed(info=1))
call criarRedeAleatoria(rede,n)
temperatura=2.0d0
beta=1.0d0/temperatura
print*,"beta:",beta
call obterVizinhanca(rede,n,vizinhos)
print*,"REDE"
do i=1,n
     print*,(rede(i,j),j=1,n)
 end do
print*,"M=",ABS(REAL(SUM(rede))/REAL(n*n))*100
!Obtenção da energia inicial
! print *,"Energias"
! do i=1,n
!     print*,(obterEnergia(rede(i,j),vizinhos(i,j,:)),j=1,n)
! end do
!Energia inicial
do i = 1, n, 1
    do j = 1, n, 1
        energia=energia+obterEnergia(rede(i,j),vizinhos(i,j,:))
    end do
end do
Print*,"Energia total inicial:", energia
!Tentar Flipar os spins e colocar a energia de cada tentativa em uma série temporal
do k=1,quantidade_loops
   x=igrnd(1,n)
   y=igrnd(1,n)
   call fliparSpin(x,y,rede,n,vizinhos,beta,dE)
end do
energia=0
do i = 1, n, 1
    do j = 1, n, 1
...
print*,"M=",ABS(REAL(SUM(rede))/REAL(n*n))*100
Print*,"Energia total final:", energia



DEALLOCATE(rede,vizinhos,serie_temp_energia)
end program teste

subroutine criarRedeAleatoria(lattice,n)
   use mtdefs
   use mtmod
    implicit none
    INTEGER,INTENT(IN)::n
    ...
    do coluna = 1, n
      ...
      lattice(linha,coluna)=x
    end do
    end do
    RETURN
end subroutine criarRedeAleatoria

subroutine obterVizinhanca(lattice,size_lattice,neighbor_list)
    implicit none
    INTEGER, INTENT(IN):: lattice(size_lattice,size_lattice)
    INTEGER,INTENT(INOUT):: neighbor_list(size_lattice,size_lattice,4)
    ...
    do j=1,size_lattice
        ! Vizinho da direita
        if(j+1<=size_lattice) then
            neighbor_list(i,j,1)=lattice(i,j+1)
        else 
            neighbor_list(i,j,1)=lattice(i,1)
        end if
        ! Vizinho da esquerda
        if(j-1==0) then
            neighbor_list(i,j,2)= lattice(i,size_lattice)
        else
            neighbor_list(i,j,2)=lattice(i,j-1)
        end if
        !Vizinho de cima
        if(i-1==0) then
            neighbor_list(i,j,3)=lattice(size_lattice,j)
        else
            neighbor_list(i,j,3)=lattice(i-1,j)
        end if
        !Vizinho de baixo
        if(i+1>size_lattice) then
            neighbor_list(i,j,4)=lattice(1,j)
        else
            neighbor_list(i,j,4)=lattice(i+1,j)
        end if
    end do
    end do
    RETURN 
end subroutine obterVizinhanca
function obterEnergia(spin,vizinhos_proximos)
    implicit none
    INTEGER,INTENT(IN)::vizinhos_proximos(4),spin
    real*8::energia,obterEnergia
    integer:: i,n
    energia= 0;
    do i=1,4
        energia = energia+vizinhos_proximos(i)
    end do
        energia = -spin*energia
        obterEnergia=energia
end function
subroutine fliparSpin(x,y, lattice,n,vizinhos, beta,dE)
    implicit none
    INTEGER, INTENT(IN)::x,y,n
    INTEGER, INTENT(INOUT)::lattice(n,n),vizinhos(n,n,4)
    REAL*8,INTENT(OUT)::dE
    REAL*8,INTENT(IN)::beta
    Real*8::obterEnergia
    INTEGER::spin
    Real::numero_aleatorio,fator_boltzman
    spin=lattice(x,y)
    dE = obterEnergia(-spin,vizinhos(x,y,:))-obterEnergia(spin,vizinhos(x,y,:))
    fator_boltzman = EXP(-beta*dE)
    CALL RANDOM_NUMBER(numero_aleatorio)
    ! print*,"Diferença de energia:",dE
    ! print*,"Aleatorio:",numero_aleatorio
    ! print*,"Fator Boltzman", fator_boltzman
    IF(dE<=0)Then
        lattice(x,y)=-spin
        call obterVizinhanca(lattice,n,vizinhos)
    ELSE
        IF(fator_boltzman>= numero_aleatorio) THEN
            lattice(x,y)=-spin
        call obterVizinhanca(lattice,n,vizinhos)
            end IF
    end IF
